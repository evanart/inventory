#!/bin/bash
# Run this from inside your cloned inventory repo:
#   git clone https://github.com/emiller1984/inventory.git
#   cd inventory
#   bash setup.sh
...
mkdir -p src public worker .github/workflows
  ...
  deploy:
    ...
    steps:
      ...
      - name: Build
        run: npm run build
        env:
          VITE_API_PROXY_URL: https://inventory-api-proxy.YOUR_SUBDOMAIN.workers.dev
      ...
      - name: Add .htaccess for SPA routing
        run: |
          ...
          RewriteRule . /inventory/index.html [L]
          HTEOF
      ...
      - name: Deploy via SFTP
        ...
        with:
          ...
          local-dir: ./dist/
          server-dir: /public_html/inventory/
          protocol: ftps
          port: 21
ENDOFFILE
...
echo ""
echo "  1. npm install"
echo "  2. npm run dev        (test locally at http://localhost:5173/inventory/)"
echo "  3. Set up Cloudflare Worker (see project-structure notes)"
echo "  4. Add GitHub secrets (FTP_SERVER, FTP_USERNAME, FTP_PASSWORD)"
echo "  5. Update VITE_API_PROXY_URL in .github/workflows/deploy.yml"
echo "  6. git add . && git commit -m 'Initial setup' && git push"
echo ""