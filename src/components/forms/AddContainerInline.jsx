...
export default function AddContainerInline({ onAdd, onCancel }) {
  ...
  return (
    <div data-component="AddContainerInline" style={{ display: "flex", gap: 6, marginBottom: 8 }}>
      ...
      <button onClick={() => { if (name.trim()) onAdd(name.trim()); }} style={{ padding: "10px 14px", borderRadius: 8, border: "none", background: "#155e75", color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Add</button>
      <button onClick={onCancel} style={{ padding: "10px 12px", borderRadius: 8, border: "1px solid #ddd", background: "#fff", color: "#666", fontSize: 14, cursor: "pointer" }}>Cancel</button>
    </div>
  );
}
