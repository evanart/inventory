...
export default function Breadcrumb({ chain, onNavigate }) {
  return (
    <div data-component="Breadcrumb" style={{ display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center", marginBottom: 14, fontSize: 13 }}>
      {chain.map((node, i) => (
        <span key={node.id} style={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          <button onClick={() => onNavigate(node.id)} style={{
            ...
            padding: "6px 12px", borderRadius: 4, fontWeight: i === chain.length - 1 ? 600 : 400,
